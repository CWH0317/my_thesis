...
def opencv_cornersubpixel(img, corners_pixel):
    # 轉換為灰度影像
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # corner
    corners_pixel = np.float32(corners_pixel)
    # 迭代條件
    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    # 獲取亞像素級別的角點坐標
    corners = cv2.cornerSubPix(gray, corners_pixel, (5, 5), (-1, -1), criteria)
...
if __name__ == '__main__':
    ...
    checkcorner_image_path = os.path.join(fileDir, "cv2cornersubpixel_subpixel_level", "checkcorner")
    ...
    for i, filename in enumerate(filelist):
        ...
        cornerP1 = opencv_cornersubpixel(img, np.array([p[0][0], p[0][1]]).reshape((1, 1, 2)))
        cornerP2 = opencv_cornersubpixel(img, np.array([p[1][0], p[1][1]]).reshape((1, 1, 2)))
        cornerP3 = opencv_cornersubpixel(img, np.array([p[2][0], p[2][1]]).reshape((1, 1, 2)))        

        p[0][0] = cornerP1[0][0][0]
        p[0][1] = cornerP1[0][0][1]
        p[1][0] = cornerP2[0][0][0]
        p[1][1] = cornerP2[0][0][1]
        p[2][0] = cornerP3[0][0][0]
        p[2][1] = cornerP3[0][0][1]
    ...
    np.savez(os.path.join(fileDir, "cv2cornersubpixel_subpixel_level", "point_position_cv_subpixel.npz"), **point_position)
    ...
    df_point_position.to_csv(os.path.join(fileDir, "cv2cornersubpixel_subpixel_level", "point_position_cv_subpixel.csv"))
    #print(df_displacement)
    #print(df_wt_rotate_center)
    #df_wt_rotate_center.to_csv("wt_rotate_center_cv_subpixel.csv")
