...
# 二次
@njit
def fit_vertexshape_quadratic(xy, r):
    s = np.dot(r.T, r)
    s_inv = np.linalg.inv(s)
    a = np.dot(s_inv, np.dot(r.T, xy))
    return a 
...
@njit
def cal_fit_coe_quadratic(equation_data):
    x_1 = np.zeros((equation_data.shape[0]),dtype=float)
    y_1 = np.zeros((equation_data.shape[0]),dtype=float)
    r_1 = np.ones((equation_data.shape[0],3),dtype=float)
    for i, ele in enumerate(equation_data):
        x_1[i] = ele[0]
        y_1[i] = ele[1]
        r_1[i][1] = ele[3]
        r_1[i][2] = ele[3] ** 2
    coe_x_1 = fit_vertexshape_quadratic(x_1, r_1)
    coe_y_1 = fit_vertexshape_quadratic(y_1, r_1)
    return coe_x_1, coe_y_1
'''
# need to check again, the equation has some problem
@njit 
def intersection_subpixel_quadratic(blade_vertex, ori_blade_vertex_r, ori_equation_1_data, ori_equation_2_data):
    possible_rlist = []
    #rlist = []
    #if len(ori_equation_1_data) > 20:
        #i_len = 20
    #else:
        #i_len = len(ori_equation_1_data)
    #if len(ori_equation_2_data) > 20:
        #j_len = 20
    #else:
        #j_len = len(ori_equation_2_data)   
    i_len = len(ori_equation_1_data)    
    j_len = len(ori_equation_2_data)
    #equation_1_data = ori_equation_1_data
    #equation_2_data = ori_equation_2_data
    #print(i_len)
    #print(j_len)
    #abs_rx_ry = 1e+15
    #r_x_keep = 0
    ...
    #i_keep = 0
    #j_keep = 0    
    for i in range(i_len, 3, -1):
        for j in range(j_len, 3, -1):
            equation_1_data = reduce_data_for_linear(blade_vertex, ori_equation_1_data, i)
            equation_2_data = reduce_data_for_linear(blade_vertex, ori_equation_2_data, j)

            equation_1_data = np.array(equation_1_data)
            equation_2_data = np.array(equation_2_data)
            coe_x_1, coe_y_1 = cal_fit_coe_quadratic(equation_1_data)
            coe_x_2, coe_y_2 = cal_fit_coe_quadratic(equation_2_data)

            try:
                root_x_a = coe_x_1[2] - coe_x_2[2]
                root_x_b = coe_x_1[1] - coe_x_2[1]
                root_x_c = coe_x_1[0] - coe_x_2[0]
                q_r_x_1 = (- root_x_b + (root_x_b**2 - 4*root_x_a*root_x_c) ** 0.5) / (2 * root_x_a)
                q_r_x_2 = (- root_x_b - (root_x_b**2 - 4*root_x_a*root_x_c) ** 0.5) / (2 * root_x_a)     
                if q_r_x_1 > 0:
                    q_r_x = q_r_x_1
                else:
                    q_r_x = q_r_x_2
                ...
                break
            try:
                root_y_a = coe_y_1[2] - coe_y_2[2]
                root_y_b = coe_y_1[1] - coe_y_2[1]
                root_y_c = coe_y_1[0] - coe_y_2[0]
                q_r_y_1 = (- root_y_b + (root_y_b**2 - 4*root_y_a*root_y_c) ** 0.5) / (2 * root_y_a)
                q_r_y_2 = (- root_y_b - (root_y_b**2 - 4*root_y_a*root_y_c) ** 0.5) / (2 * root_y_a)
                if q_r_y_1 > 0:
                    q_r_y = q_r_y_1
                else :
                    q_r_y = q_r_y_2
            ...
            #rlist.append([i, j, r_x, r_y, abs(r_x - r_y)])
            #print(i, j)
            if abs(q_r_x - q_r_y) <= 10. and abs(q_r_x - ori_blade_vertex_r) <= 10. and abs(q_r_y - ori_blade_vertex_r) <= 10.:
                possible_rlist.append([q_r_x, q_r_y, coe_x_1[0], coe_x_1[1], coe_x_1[2], coe_y_1[0], coe_y_1[1], coe_y_1[2], coe_x_2[0], coe_x_2[1], coe_x_2[2], coe_y_2[0], coe_y_2[1], coe_y_2[2], abs(q_r_x - q_r_y), (q_r_x - ori_blade_vertex_r) ** 2 + (q_r_y - ori_blade_vertex_r) ** 2, (q_r_x - ori_blade_vertex_r) ** 2 + (q_r_y - ori_blade_vertex_r) ** 2 + (q_r_x - q_r_y) ** 2])
                #abs_rx_ry = abs(r_x - r_y)
                #r_x_keep = r_x
                #r_y_keep = r_y
                #i_keep = i
    ...
    f_r_x = sort_rlist[0][0]
    f_r_y = sort_rlist[0][1]
    f_coe_x_1 = [sort_rlist[0][2], sort_rlist[0][3], sort_rlist[0][4]]
    f_coe_y_1 = [sort_rlist[0][5], sort_rlist[0][6], sort_rlist[0][7]]
    f_coe_x_2 = [sort_rlist[0][8], sort_rlist[0][9], sort_rlist[0][10]]
    f_coe_y_2 = [sort_rlist[0][11], sort_rlist[0][12], sort_rlist[0][13]]
    r = (f_r_x + f_r_y) / 2
    cal_x_1 = f_coe_x_1[0] + f_coe_x_1[1] * r + f_coe_x_1[2] * r * r 
    cal_x_2 = f_coe_x_2[0] + f_coe_x_2[1] * r + f_coe_x_2[2] * r * r
    cal_y_1 = f_coe_y_1[0] + f_coe_y_1[1] * r + f_coe_y_1[2] * r * r 
    cal_y_2 = f_coe_y_2[0] + f_coe_y_2[1] * r + f_coe_y_2[2] * r * r 
    cal_x = (cal_x_1 + cal_x_2) / 2
    cal_y = (cal_y_1 + cal_y_2) / 2
    #print(f_r_x)
    #print(f_r_y)
...
def intersection_subpixel_quadratic(blade_vertex, ori_blade_vertex_r, ori_equation_1_data, ori_equation_2_data):
    ...
    #print(i_len)
    #print(j_len)
    #print(ori_equation_1_data)
    #print(ori_equation_2_data)
    #abs_rx_ry = 1e+15
    #r_x_keep = 0
    #r_y_keep = 0
    #i_keep = 0
    ...
    for i in range(i_len, 3, -1):
        for j in range(j_len, 3, -1):
            ...
            #print(i, j)
            #if abs(q_r_x - q_r_y) <= 10. and abs(q_r_x - ori_blade_vertex_r) <= 10. and abs(q_r_y - ori_blade_vertex_r) <= 10.:
            #if np.isnan(q_r_x) or np.isnan(q_r_y):
                #break
            #else:
            if not (np.isnan(q_r_x) or np.isnan(q_r_y)):
                possible_rlist.append([q_r_x, q_r_y, coe_x_1[0], coe_x_1[1], coe_x_1[2], coe_y_1[0], coe_y_1[1], coe_y_1[2], coe_x_2[0], coe_x_2[1], coe_x_2[2], coe_y_2[0], coe_y_2[1], coe_y_2[2], abs(q_r_x - q_r_y), (q_r_x - ori_blade_vertex_r) ** 2 + (q_r_y - ori_blade_vertex_r) ** 2, (q_r_x - ori_blade_vertex_r) ** 2 + (q_r_y - ori_blade_vertex_r) ** 2 + (q_r_x - q_r_y) ** 2])
                #abs_rx_ry = abs(r_x - r_y)
                #r_x_keep = r_x
                #r_y_keep = r_y
    ...
    # check point
    if ((cal_x - blade_vertex[0]) ** 2 + (cal_y - blade_vertex[1]) ** 2 ) ** 0.5 >= 2.:
        f_cal_x = blade_vertex[0]
        f_cal_y = blade_vertex[1]
    else:
        f_cal_x = cal_x
        f_cal_y = cal_y
    return f_cal_x, f_cal_y

def find_intersection_subpixel_quadratic(blade_vertex_point, sorted_possible_vertex_list):
    # 用方位角先預選葉片頂點周圍點位
    preselect_vertex_list = []
    for ele in sorted_possible_vertex_list:
        if abs(blade_vertex_point [2]- ele[2]) < 60:
    ...
    for ele in ori_data:
        if ele[0] == blade_vertex[0] and ele[1] == blade_vertex[1]:
            ori_blade_vertex_r = ele[3]
    #print(ori_blade_vertex_r)
    cal_x, cal_y = intersection_subpixel_quadratic(blade_vertex, ori_blade_vertex_r, equation_1_data, equation_2_data)
    #intersection_subpixel(blade_vertex, equation_1_data, equation_2_data)
...
if __name__ == '__main__':
    ...
    checkcorner_image_path = os.path.join(fileDir, "intersection_quadratic_subpixel_level", "checkcorner")
    ...
    for i, filename in enumerate(filelist):
        ...
        cal_x_p1, cal_y_p1 = find_intersection_subpixel_quadratic(p[0], vertex_around_p_list)
        cal_x_p2, cal_y_p2 = find_intersection_subpixel_quadratic(p[1], vertex_around_p_list)
        cal_x_p3, cal_y_p3 = find_intersection_subpixel_quadratic(p[2], vertex_around_p_list)     
        ...
        estimated_time = elapsed_time / completed_iterations * (total_iterations - completed_iterations)
        
        # Print progress information
        print(f"Step {completed_iterations}/{total_iterations} - Elapsed Time: {elapsed_time:.2f}s - Estimated Time Remaining: {estimated_time:.2f}s")
    '''
    ...
    np.savez(os.path.join(fileDir, "intersection_quadratic_subpixel_level", "point_position_quadratic_intersectionsubpixel.npz"), **point_position)
    ...
    df_point_position.to_csv(os.path.join(fileDir, "intersection_quadratic_subpixel_level", "point_position_quadratic_intersectionsubpixel.csv"))
